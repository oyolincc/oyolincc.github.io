...
在[上文runtime-dom/src/renderer.ts]中可知render方法走了`patch`，来看看代码：
...
关于关于注释节点、文本vnode逻辑没啥好说的。重点是 processElement和processComponent，如果没有旧vnode就直接渲染就行，是element就创建DOM，是component就创建组件实例。
